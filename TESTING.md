...
* When updating the booking, if the user wants to change something in the booking, then he has to choose a new time also because otherwise, it is not possible to finish the update. The problem is caused by the function that checks if the user has chosen available time that retrieves time data from other bookings in the database, among them he also picks up the booking that is being updated. The lack of time prevented me from solving this problem because the existing bug was discovered on the day of the project submission. 
